...
def offset_handle_pull_pose(
    pose: Any, cabinet_root_pose: Any, vertical_offset_m: float
) -> np.ndarray:
    """Offset the pull endpoint along the cabinet's semantic up axis.

    A signed correction keeps closing fingers in the measured handle plane as
    the arm approaches its inner-workspace limit.  Expressing the offset in the
    cabinet frame preserves the meaning for rotated assets.
    """
    result = _pose(pose, "pose").copy()
    root = _pose(cabinet_root_pose, "cabinet_root_pose")
    if not np.isfinite(vertical_offset_m):
        raise ValueError("vertical_offset_m must be finite")
    result[:3] += quaternion_rotate(root[3:], [0.0, 0.0, 1.0]) * float(
        vertical_offset_m
    )
    return result
