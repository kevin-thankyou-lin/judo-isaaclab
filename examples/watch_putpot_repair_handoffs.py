...
def _prompt(boundary: dict[str, Any]) -> str:
    if boundary["action"] == "diagnose_and_repair":
        return (
            ...
            f"inspect {boundary['latest_result_json']}, its trace, and the "
            "synchronized skill.mp4 before diagnosing the physics failure. Compare "
            "the latest cycle with the prior cycle when one exists. Before "
            "submitting, state a compact repair contract: FIRST_FAILED_STAGE, "
            "PRIMARY_METRIC_BEFORE_AND_AFTER, one falsifiable HYPOTHESIS, "
            "EXPECTED_DELTA, REPAIR_FAMILY, and SCOPE=local_or_structural. "
            "Before attributing a missed grasp to either wrist, reconstruct the "
            "full acquisition timeline from synchronized video and trace evidence: "
            "FIRST_CONTACT_ARM, FIRST_CONTACT_STEP, CONTACT_ORDER, "
            "PREGRASP_OBJECT_MOTION, OBJECT_MOTION_BEFORE_PEER_CONTACT, and "
            "per-pad force/contact windows. If the object moves materially before "
            "the peer arm arrives, treat the earliest contacting arm and its "
            "approach/sequencing as the upstream cause; do not tune the downstream "
            "wrist against a target that the first arm already displaced. State an "
            "evidence-derived object-motion abort threshold, fail closed when it is "
            "exceeded, and prefer a safer standoff, contact-gated stop/backoff, "
            "right/left-first ordering, synchronized acquisition, or observed-pose "
            "reanchoring as justified by the trace. Do not begin transport until "
            "both hands have sustained dual-pad contact for a stated validation "
            "window. A "
            "different spec or controller hash proves only that code changed; it "
            "does not prove physical progress. Use a local parameter or feedback "
            "repair only when the video and trace show that the upstream semantic "
            "approach/contact frame is correct and the observed tracking residual "
            "is within the repair's authority. If two attempts in one repair "
            ...
            "rotate if no such change is justified within the cycle budget. Then "
            "submit exactly one evidence-linked revised Python controller plugin "
            "and/or observed program spec through "
            ...
            "spend another cycle on an exhausted repair family, weaken acceptance "
            "gates, or restart Isaac. Recheck the queue first and do nothing "
            "duplicate if another turn already handled this receipt."
        )
    return (
