...
def _measured_right_dual_contact_pad_balance(
    ...
    *,
    ...
    if any(
        f"/lanes/{lane_id}/" not in str(path)
        for path in (trace_source, diagnosis_source)
    ):
        raise ValueError("right pad balance evidence must be pair-lane local")
    limits = np.asarray(
        [
            minimum_force_n,
            minimum_pad_fraction_margin,
            target_weak_pad_fraction,
            maximum_translation_m,
        ],
        dtype=np.float64,
    )
    if (
        ...
        raise ValueError("right pad balance limits are invalid")
    diagnosis = json.loads(diagnosis_source.read_text(encoding="utf-8"))
    failure = diagnosis.get("earliest_causal_failure", {})
    expected_forces = np.asarray(
        failure.get("attempt_62_first_right_dual_force_forces_n", []),
        dtype=np.float64,
    )
    ...
    with np.load(trace_source, allow_pickle=False) as trace:
        ...
        if bool(np.asarray(trace["partial_trace"]).reshape(())):
            raise ValueError("right pad balance requires a complete trace")
        if not 0 <= int(sample_step) < len(trace["right_finger_forces_n"]):
            raise ValueError("right pad balance sample is outside the trace")
        forces = np.asarray(
            trace["right_finger_forces_n"][sample_step], dtype=np.float64
        )
        fractions = np.asarray(
            trace["right_pad_fractions"][sample_step], dtype=np.float64
        )
        axes = np.asarray(
    ...
    if (
        ...
        raise ValueError("right pad balance trace does not match the diagnosis")
    weak = int(np.argmin(fractions))
    strong = 1 - weak
    if not (
        fractions[weak] < minimum_pad_fraction_margin
        and minimum_pad_fraction_margin
        <= fractions[strong]
        <= 1.0 - minimum_pad_fraction_margin
    ):
    ...
    return corrected, {
        ...
        "diagnosis": {
            "path": str(diagnosis_source),
            "sha256": _sha256(diagnosis_source),
        },
        "finger_forces_n": forces.tolist(),
        "pad_fractions_before": fractions.tolist(),
        "weak_finger_index": weak,
...
def _parser(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Pair-lane diagnosis owning the measured right sample.",
    )
    parser.add_argument(
        "--target-right-quality-precontact-pad-balance-step",
        type=int,
        help="Sample step in the measured right pad-balance trace.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--target-left-quality-loaded-pad-pivot-closure",
        action="store_true",
        help=(
...
def main(argv: list[str] | None = None) -> None:
    ...
    right_precontact_pad_balance_values = (
        args.target_right_quality_precontact_pad_balance_trace,
        args.target_right_quality_precontact_pad_balance_diagnosis_json,
        args.target_right_quality_precontact_pad_balance_step,
    )
    right_precontact_pad_balance_requested = any(
        value is not None for value in right_precontact_pad_balance_values
    )
    if right_precontact_pad_balance_requested and not all(
        value is not None for value in right_precontact_pad_balance_values
    ):
        raise ValueError(
            "right precontact pad balance requires a pair-local trace, "
            "diagnosis, and step"
        )
    if right_precontact_pad_balance_requested and not (
        args.target_right_quality_postclosure_force_settle
        and args.target_collision_clear_right_pregrasp
    ):
        raise ValueError(
            "right precontact pad balance requires the collision-clear right "
            "pregrasp and post-closure quality safeguards"
        )
    if args.target_left_quality_pre_peer_motion_budgeted_closure and not (
        ...
        raise ValueError(
            "left pre-peer motion-budgeted closure requires sequential quality "
            "MPC with committed closure and the dual-force pad-margin pivot"
        )
    ...
    if args.target_left_quality_loaded_pad_pivot_closure and not (
        args.target_left_quality_transverse_aligned_two_pad_closure
        and args.target_left_quality_handle_normal_loaded_pad_pivot
    ):
        raise ValueError(
    ...
    try:
        ...
        right_precontact_pad_balance = None
