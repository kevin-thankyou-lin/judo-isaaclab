...
from judo_isaaclab import hang_mug_acceptance
...
def _parser() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Require exact collision auditing against the configured contact budget.",
    )
...
def main() -> None:
    ...
    try:
        ...
        clean_insertion = hang_mug_acceptance.audit_clean_insertion(
            args,
            keyframes=keyframes,
        ...
        if clean_insertion is not None:
            print(
        ...
        controller_gain_contract = hang_mug_acceptance.controller_gain_receipt(args)
        hang_mug_acceptance.add_quality_checks(
            checks,
            args=args,
            clean_insertion=clean_insertion,
            gain_receipt=controller_gain_contract,
        )
        ...
        if args.classification_run:
            ...
            acceptance = hang_mug_acceptance.add_quality_acceptance(
                acceptance, args=args, checks=checks
            )
        ...
        result = {
            ...
            "mode": args.mode,
            "protocol": {"controller": controller, "candidate_sampling": False, "scene_resets": protocol_receipt["environment_resets"], "initial_state_restores": protocol_receipt["initial_state_restores"], "inter_stage_resets": protocol_receipt["inter_stage_resets"], "teleports_after_reset": protocol_receipt["teleports_after_rollout_start"], "execution_instrumentation": protocol_receipt, "semantic_execution_hooks": {"enabled": execution_hooks.enabled, "policy": "optional_observation_only"}, "dataset_object_aliases": object_aliases, "dataset_alias_policy": "load_time_keys_only_source_bytes_unchanged", "control_rate_hz": 30, "steps": len(actions), "seed": args.seed, "physics_device_requested": args.device, "physics_device_actual": str(env.device), "physics_device_requirement": "cpu" if args.require_cpu_physics else None, "physics_device_receipt": physics_device, "grasp_assistance": grasp_assistance, "planned_clean_insertion": planned_clean_insertion, "clean_insertion": clean_insertion, "controller_gain_contract": controller_gain_contract, "parameters": {"damping": args.damping, "max_joint_delta": args.max_joint_delta, "max_position_step": args.max_position_step, "max_rotation_step": args.max_rotation_step, "insert_clearance_m": args.insert_clearance_m, "clean_insertion_tracking_margin_m": args.clean_insertion_tracking_margin_m, "replay_hang_insert_dls_gain": args.replay_hang_insert_dls_gain, "replay_hang_support_dls_gain": args.replay_hang_support_dls_gain, "replay_hang_support_gain_lead_steps": args.replay_hang_support_gain_lead_steps, "replay_hang_insert_time_scale": args.replay_hang_insert_time_scale, "handover_contact_settle_steps": args.handover_contact_settle_steps, "source_action_prefix_steps": repair_prefix_steps, "replay_target_branch_policy": args.replay_target_branch_policy, "observed_left_anchor_held_during_handover": observed_handover_reanchor, "observed_handover_reanchor": observed_handover_reanchor, "right_contact_feedback_reanchor": trajectory is not None, "pick_clearance_uses_measured_body_height": True, "mug_body_frame_scaling": True, "handle_hole_branch_frame_transfer": True, "branch_support_midpoint": True}},
