...
def _parser() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--post-release-return-uniform-interpolation",
        action="store_true",
        help=(
            "Sample only the screened open-arm return uniformly so it gains "
            "clearance immediately; preserve its straight segment and endpoint."
        ),
    )
    parser.add_argument(
        ...
        default=0,
        help=(
            "Bounded rows to hold the supported-release orientation while the "
            "same direct return translation begins."
        ),
    )
    parser.add_argument(
        ...
        default=0,
        help=(
            "Rows to hold the demonstrated-rest endpoint inside the same open "
            "post-release return phase after its direct interpolation."
        ),
    )
    parser.add_argument(
...
def _build_skill(
    ...
    if direct_contract:
        ...
        program.release_and_return_to_rest(
            ...
            uniform_return=args.post_release_return_uniform_interpolation,
            return_orientation_delay_rows=(
                args.post_release_return_orientation_delay_rows
            ),
            return_endpoint_hold_rows=(
                args.post_release_return_endpoint_hold_steps
            ),
        )
    else:
...
def main() -> None:
    ...
    if args.post_release_return_uniform_interpolation and not direct_steps[1]:
        raise ValueError(
            "uniform post-release return requires direct return choreography"
        )
    orientation_delay = args.post_release_return_orientation_delay_rows
    if (
        isinstance(orientation_delay, bool)
        or not 0 <= orientation_delay <= 4
        or orientation_delay >= direct_steps[1]
        and orientation_delay != 0
    ):
        raise ValueError(
            "post-release return orientation delay must be in [0, 4] and shorter than the return"
        )
    if orientation_delay and not args.post_release_return_uniform_interpolation:
        raise ValueError(
            "post-release return orientation delay requires uniform interpolation"
        )
    endpoint_hold = args.post_release_return_endpoint_hold_steps
    if (
        isinstance(endpoint_hold, bool)
        or not 0 <= endpoint_hold <= 60
    ):
        raise ValueError(
            "post-release return endpoint hold steps must be in [0, 60]"
        )
    if endpoint_hold and not direct_steps[1]:
        raise ValueError(
            "post-release return endpoint hold requires direct return choreography"
        )
        ...
        for step in rollout_steps:
            ...
            if semantic_step is not None and any(
                ...
                trajectory = reanchor_branch_transport_contact(
                    ...
                    uniform_post_release_return=(
                        args.post_release_return_uniform_interpolation
                    ),
                    post_release_return_orientation_delay_rows=(
                        args.post_release_return_orientation_delay_rows
                    ),
                    post_release_return_endpoint_hold_rows=(
                        args.post_release_return_endpoint_hold_steps
                    ),
                )
                total_steps = _rollout_total_steps(
