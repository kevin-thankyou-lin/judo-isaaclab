...
def _parser() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--post-release-return-motion-steps",
        type=int,
        default=0,
        help=(
            "Optional rows used by the direct open-arm return motion; any "
            "remaining return rows hold the demonstrated rest endpoint."
        ),
    )
    parser.add_argument(
...
def _handover_target_with_local_roll(pose, angle_rad: float) -> np.ndarray:
    """Rotate a receiver waypoint about its own X axis without translating it."""
    from judo_isaaclab.put_marker import _pose, quaternion_multiply

    angle = float(angle_rad)
    if not np.isfinite(angle) or abs(angle) > np.pi / 4.0:
        raise ValueError(
            "handover target local roll must be finite and within 45 degrees"
        )
    result = _pose(pose, "handover target").copy()
    half = 0.5 * angle
    result[3:] = quaternion_multiply(
...
def _build_skill(
    ...
    if direct_contract:
        ...
        program.release_and_return_to_rest(
            ...
            return_motion_steps=(args.post_release_return_motion_steps or None),
...
def main() -> None:
    ...
    return_motion_steps = args.post_release_return_motion_steps
    if (
        isinstance(return_motion_steps, bool)
        or not 0 <= return_motion_steps <= direct_steps[1]
        or return_motion_steps and not direct_steps[1]
    ):
        raise ValueError(
            "post-release return motion steps must be in [0, return steps]"
        )
