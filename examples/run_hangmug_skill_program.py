...
def _parser() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=0.0,
        help=(
            "Bounded receiver-local Y orientation clearance applied only to "
            "the open direct-return virtual start."
        ),
    )
    parser.add_argument(
...
def main() -> None:
    ...
    return_clearance = float(args.post_release_return_local_y_clearance_rad)
    if not np.isfinite(return_clearance) or abs(return_clearance) > 0.16:
        raise ValueError(
            "post-release return local-Y clearance must be within 0.16 rad"
        )
    if return_clearance and not direct_steps[1]:
        raise ValueError(
            ...
            if semantic_step is not None and any(
                ...
                trajectory = reanchor_branch_transport_contact(
                    ...
                    post_release_return_local_y_clearance_rad=(
                        args.post_release_return_local_y_clearance_rad
                    ),
                )
                total_steps = _rollout_total_steps(
                    trajectory,
