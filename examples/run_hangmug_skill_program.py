...
def _parser() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=0.0,
        help=(
            "Bounded extra open-gripper travel along the demonstrated pickup "
            "approach axis before closing."
        ),
    )
    parser.add_argument(
...
def _advance_pick_contact(pregrasp, grasp, advance_m: float) -> np.ndarray:
    """Move the pickup target deeper along its demonstrated approach axis."""
    ...
    if (
        ...
        raise ValueError("pick pregrasp and grasp poses must be finite seven-value poses")
    advance = float(advance_m)
    if not np.isfinite(advance) or not 0.0 <= advance <= 0.015:
    ...
    if distance <= 1.0e-9:
        raise ValueError("pick approach axis must have nonzero translation")
    advanced = grasp_pose.copy()
    advanced[:3] += approach / distance * advance
    return advanced
...
def _build_skill(
    ...
    left_grasp = _advance_pick_contact(
        left_pregrasp,
        transfer_mug_frame("left_grasp", "left"),
        args.pick_approach_advance_m,
    )
...
def main() -> None:
    ...
    try:
        ...
        result = {
            ...
            "pair_geometry": {
                "pick_approach_advance_m": float(args.pick_approach_advance_m),
            },
