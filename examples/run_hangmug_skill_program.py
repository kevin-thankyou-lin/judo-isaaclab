...
def _parser() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=0.0,
        help=(
            "Bounded receiver-local +Z roll about the target handle-hole point; "
            "positive is clockwise when viewed from wrist toward mug."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--right-release-clear-local-pitch-rad",
        type=float,
        default=0.0,
        help=(
            "Bounded receiver-local Y pitch applied only during the single final "
            "supported opening, before the direct open-arm return."
        ),
    )
    parser.add_argument(
...
def _handover_target_with_local_contact_pivot_roll(
    pose, pivot_world, angle_rad: float
) -> np.ndarray:
    """Roll the receiver about local +Z while preserving a world contact point."""
    from judo_isaaclab.put_marker import _pose, quaternion_multiply, quaternion_rotate

    angle = float(angle_rad)
    if not np.isfinite(angle) or abs(angle) > np.pi / 2.0:
        raise ValueError("handover target local roll must be finite and within 90 degrees")
    result = _pose(pose, "handover target").copy()
    pivot = np.asarray(pivot_world, dtype=np.float64)
    if pivot.shape != (3,) or not np.all(np.isfinite(pivot)):
        raise ValueError("handover contact pivot must contain three finite values")
    conjugate = result[3:] * np.asarray([1.0, -1.0, -1.0, -1.0])
    local_pivot = quaternion_rotate(conjugate, pivot - result[:3])
    half = 0.5 * angle
    result[3:] = quaternion_multiply(
        result[3:], np.asarray([np.cos(half), 0.0, 0.0, np.sin(half)])
    )
    result[3:] /= np.linalg.norm(result[3:])
    result[:3] = pivot - quaternion_rotate(result[3:], local_pivot)
    return result


def _right_release_clear_with_local_pitch(
    pose, pivot_world, baseline_local_roll_rad: float, pitch_rad: float
) -> np.ndarray:
    """Pitch only the final opening pose in the receiver's pre-roll frame."""

    unrolled = _handover_target_with_local_contact_pivot_roll(
        pose, pivot_world, -float(baseline_local_roll_rad)
    )
    pitched = _handover_target_with_local_pitch(unrolled, pitch_rad)
    return _handover_target_with_local_contact_pivot_roll(
        pitched, pivot_world, float(baseline_local_roll_rad)
    )
...
def _build_skill(
    ...
    target_handle_world = compose_pose(
        target_handover_mug.root_pose, target_parts.handle_hole_frame
    )
    right_grasp = _handover_target_with_local_contact_pivot_roll(
        right_grasp,
        target_handle_world[:3],
        args.handover_target_local_roll_rad,
    )
    ...
    right_release_clear = right_insert
    if args.right_release_clear_local_pitch_rad:
        final_handle_world = compose_pose(
            final_mug.root_pose, target_parts.handle_hole_frame
        )
        right_release_clear = _right_release_clear_with_local_pitch(
            right_insert,
            final_handle_world[:3],
            args.handover_target_local_roll_rad,
            args.right_release_clear_local_pitch_rad,
        )
    ...
    if direct_contract:
        ...
        program.release_and_return_to_rest(
            right_insert,
            right_start,
            right_release_pose=right_release_clear,
            support_steps=40,
            release_steps=40,
...
def main() -> None:
    ...
    if args.handover_target_local_roll_rad and not args.handover_orient_steps:
        raise ValueError("handover local roll requires orient-first descent")
    if args.handover_target_local_roll_rad and not args.handover_handle_frame_transfer:
        raise ValueError("handover local roll requires handle-frame transfer")
    if (
        args.right_release_clear_local_pitch_rad
        and not args.direct_rest_to_preinsert_steps
    ):
