...
def _parser() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "Rows that hold the row-seven clearance orientation while the same "
            "straight open return translates farther from the tree, before "
            "completing the original shortest-arc rotation to rest."
        ),
    )
    parser.add_argument(
        ...
        nargs=3,
        help=(
            "Optional release-frame rotation vector for the bounded open-return "
            "brake target; requires rotation-hold rows."
        ),
    )
    parser.add_argument(
        "--post-release-return-observed-reanchor",
        action="store_true",
        help=(
            "Regenerate the one direct open return from the last observed "
            "fully-open release pose."
        ),
    )
    parser.add_argument(
...
def _update_authored_assist_releases(env, trajectory, step: int) -> None:
    ...
    The task manager keeps the left giver assist engaged through receiver
    overlap and disables it with giver release.  The semantic program's
    deterministic left-open boundary is the authored release signal; receiver
    contact alone never advances the giver assist state machine.
    """
    import torch
...
def _build_skill(
    ...
    if direct_contract:
        from judo_isaaclab.hang_mug import (
            hold_post_release_return_rotation_after_clearance,
        )

        trajectory = hold_post_release_return_rotation_after_clearance(
            trajectory,
            args.post_release_return_rotation_hold_steps,
            brake_rotation_vector=(
                args.post_release_return_brake_rotation_vector
            ),
        )
    return (
        trajectory,
...
def main() -> None:
    ...
    rotation_hold_steps = args.post_release_return_rotation_hold_steps
    if (
    ...
    brake_rotation_vector = args.post_release_return_brake_rotation_vector
    ...
    if args.post_release_return_observed_reanchor and not direct_steps[1]:
        raise ValueError(
            "post-release observed reanchor requires direct choreography"
        )
    ...
    try:
        ...
        for step in rollout_steps:
            ...
            if (
                args.post_release_return_observed_reanchor
                and semantic_step is not None
                and "post_release_return" in trajectory.waypoint_steps
                ...
                trajectory = reanchor_open_return_from_observed_release(
                    trajectory,
        ...
        result = {
            ...
            "handover_assist_lifecycle": {
                "release_policy": "left_giver_release",
                "receiver_overlap_does_not_release_giver_assist": True,
            },
            "post_handover_setup": {
