...
def _handover_orient_clear_pose(
    pose, world_z_m: float, local_z_m: float
) -> np.ndarray:
    ...
    result = _pose(pose, "handover target").copy()
    result[2] += world
    result[:3] -= quaternion_rotate(result[3:], [0.0, 0.0, local])
    return result
