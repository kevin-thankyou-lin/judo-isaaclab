...
def _parser() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--branch-support-handle-offset-m",
        type=float,
        nargs=3,
        default=(0.0, 0.0, 0.0),
        help=(
...
def _bounded_branch_support_handle_offset(value) -> np.ndarray:
    offset = np.asarray(value, dtype=np.float64)
    if (
        offset.shape != (3,)
        or not np.all(np.isfinite(offset))
        or np.linalg.norm(offset) > 0.02
    ):
        raise ValueError(
...
def _build_skill(
    ...
    if args.handover_orient_steps:
        ...
        right_orient_clear = right_grasp.copy()
        right_orient_clear[2] += args.handover_orient_clearance_m
...
def main() -> None:
    ...
    _bounded_branch_support_handle_offset(args.branch_support_handle_offset_m)
