...
def _parser() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=(0.0, 0.0, 0.0),
        metavar=("X", "Y", "Z"),
        help="Bounded world-frame translation of the final supported mug pose.",
    )
    parser.add_argument(
...
def _bounded_branch_final_offset(value) -> np.ndarray:
    offset = np.asarray(value, dtype=np.float64)
    if offset.shape != (3,) or not np.all(np.isfinite(offset)):
        raise ValueError("branch final offset must contain three finite values")
    if np.linalg.norm(offset) > 0.01:
        raise ValueError("branch final offset exceeds 1 cm")
    return offset
...
def _build_skill(
    ...
    final_mug_pose[:3] += _bounded_branch_final_offset(args.branch_final_offset_m)
...
def main() -> None:
    ...
    _bounded_branch_final_offset(args.branch_final_offset_m)
