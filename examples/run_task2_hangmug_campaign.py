    ...
    if selection["actual_repair_boundary"] == "reset" or handover_strategy:
        ...
        if "handover_target_offset_m" in strategy:
            arguments.extend([
                ...
                *(f"{float(value):.17f}" for value in strategy["handover_target_offset_m"]),
            ])
...
def _repair_strategy(index: int) -> dict:
    ...
    acquire_stationary = value.get("handover_contact_acquire_stationary", False)
    ...
    if acquire_stationary and (not acquire or np.any(acquire_bias)):
        raise ValueError(
    ...
    if sum(
        bool(item) for item in (orient_clearance, orient_local_y, orient_local_z)
    ) > 1:
    ...
    if (
        ...
        or isinstance(orient_local_y, bool)
        or not isinstance(orient_local_y, (int, float))
        or not np.isfinite(orient_local_y)
        or abs(orient_local_y) > 0.12
        ...
        or bool(orient_clearance or orient_local_y or orient_local_z)
        != bool(orient_steps)
    ):
        raise ValueError(
    ...
    if orient_steps:
        ...
        if orient_local_y:
            strategy["handover_orient_local_y_clearance_m"] = float(orient_local_y)
        if orient_local_z:
