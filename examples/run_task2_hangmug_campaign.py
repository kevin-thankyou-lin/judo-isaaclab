...
def _repair_command(
    ...
    if "branch_support_handle_offset_m" in strategy:
        arguments.extend([
            "--branch-support-handle-offset-m",
            *map(str, strategy["branch_support_handle_offset_m"]),
        ])
...
def _repair_strategy(index: int) -> dict:
    ...
    if "branch_support_handle_offset_m" in value:
        offset = np.asarray(value["branch_support_handle_offset_m"], dtype=float)
        if (
            offset.shape != (3,)
        ...
        strategy["branch_support_handle_offset_m"] = offset.tolist()
    ...
    if (
        ...
        or not isinstance(roll, (int, float))
        or not np.isfinite(roll)
        or abs(roll) > np.pi / 2.0
    ):
        raise ValueError("handover target local roll must be within 90 degrees")
    if (
    ...
    if "handover_target_local_roll_rad" in value:
        if value.get("handover_handle_frame_transfer") is not True:
...
def _worker_pids() -> list[int]:
    ...
    for process in Path("/proc").glob("[0-9]*"):
        try:
            ...
            continue
        if comm not in {"isaac-sim", "kit"} and "run_hangmug_skill_program.py" not in names:
            continue
        if lane_id is not None:
            try:
                environment = process.joinpath("environ").read_bytes().split(b"\0")
            except (FileNotFoundError, PermissionError, ProcessLookupError):
                # A candidate worker whose lane cannot be proven is unsafe.
                workers.append(int(process.name))
                continue
            expected = f"CPGEN_LANE_ID={lane_id}".encode()
