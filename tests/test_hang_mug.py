...
def test_handover_local_roll_rotates_pad_axis_without_moving_or_changing_closing_axis():
    pose = _handover_target_with_local_pitch(_pose(0.4, -0.1, 0.9), 0.52)
    rotated = _handover_target_with_local_roll(pose, 0.4)
    np.testing.assert_allclose(rotated[:3], pose[:3], atol=0.0)
    np.testing.assert_allclose(
        quaternion_rotate(rotated[3:], [1.0, 0.0, 0.0]),
        quaternion_rotate(pose[3:], [1.0, 0.0, 0.0]),
        atol=1.0e-12,
    )
    assert not np.allclose(
        quaternion_rotate(rotated[3:], [0.0, 0.0, 1.0]),
        quaternion_rotate(pose[3:], [0.0, 0.0, 1.0]),
    )
    with pytest.raises(ValueError, match="within 45 degrees"):
        _handover_target_with_local_roll(pose, np.pi / 4.0 + 1.0e-6)
...
def test_contact_acquire_can_target_evidence_backed_mug_frame_pose():
    program = HangMugSkillProgram(_pose(), _pose())
    program.physical_handover(
        _pose(), _pose(), _pose(), _pose(),
        approach_steps=1,
    ...
    observed_contact = compose_pose(inverse_pose(mug), observed_right)
    angle = 0.01
    target_quaternion = compose_pose(
        np.asarray([0.0, 0.0, 0.0, np.cos(angle / 2), 0.0, 0.0, np.sin(angle / 2)]),
        observed_contact,
    )[3:]
    target_position = observed_contact[:3] + [0.001, -0.002, 0.003]
    target_contact = np.concatenate((target_position, target_quaternion))
    target_mug = compose_pose(observed_right, inverse_pose(target_contact))
    ...
    adjusted, receipt = reanchor_handover_contact_acquire(
        ...
        observed_right,
        target_contact_mug_position_m=target_position,
        target_contact_mug_quaternion_wxyz=target_quaternion,
    )

    grasp_end = adjusted.waypoint_steps["right_grasp"]
    acquire_end = adjusted.waypoint_steps["handover_contact_acquire"]
    np.testing.assert_allclose(receipt["target_contact_mug_frame"], target_contact)
    np.testing.assert_allclose(receipt["target_mug_world"], target_mug)
    assert receipt["rotation_error_rad"] == pytest.approx(angle)
    assert receipt["orientation_unchanged"] is False
    np.testing.assert_allclose(
        adjusted.left_poses[acquire_end],
...
def test_contact_acquire_can_stop_at_an_evidence_backed_interpolation_prefix():
    ...
    observed_right = _pose(0.4, -0.1, 0.9)
    observed_contact = compose_pose(inverse_pose(mug), observed_right)
    target_contact = observed_contact.copy()
    target_contact[:3] += [0.001, -0.002, 0.003]
    angle = 0.01
    target_contact[3:] = compose_pose(
        np.asarray([0.0, 0.0, 0.0, np.cos(angle / 2), 0.0, 0.0, np.sin(angle / 2)]),
        observed_contact,
    )[3:]

    full, _ = reanchor_handover_contact_acquire(
        trajectory(24), _pose(), mug, observed_left, observed_right,
        target_contact_mug_position_m=target_contact[:3],
        target_contact_mug_quaternion_wxyz=target_contact[3:],
    )
    prefix, receipt = reanchor_handover_contact_acquire(
        trajectory(21), _pose(), mug, observed_left, observed_right,
        target_contact_mug_position_m=target_contact[:3],
        target_contact_mug_quaternion_wxyz=target_contact[3:],
        interpolation_reference_steps=24,
    )
    full_start = full.waypoint_steps["right_grasp"] + 1
    prefix_start = prefix.waypoint_steps["right_grasp"] + 1
    np.testing.assert_allclose(
        prefix.left_poses[prefix_start : prefix_start + 21],
        full.left_poses[full_start : full_start + 21],
    )
    np.testing.assert_allclose(
        prefix.right_poses[prefix_start : prefix_start + 21],
        full.right_poses[full_start : full_start + 21],
    )
    assert receipt["interpolation_reference_steps"] == 24
    assert receipt["interpolation_endpoint_fraction"] == pytest.approx(21 / 24)
    assert receipt["acquisition_endpoint_contact_mug_frame"] != pytest.approx(
        receipt["target_contact_mug_frame"]
    )


def test_contact_acquire_rejects_reference_shorter_than_execution():
    program = HangMugSkillProgram(_pose(), _pose())
    program.physical_handover(
        _pose(), _pose(), _pose(), _pose(),
        approach_steps=1,
    ...
    with pytest.raises(ValueError, match="reference steps"):
        reanchor_handover_contact_acquire(
            program.build(), _pose(), _pose(), _pose(), _pose(),
            interpolation_reference_steps=2,
        )
