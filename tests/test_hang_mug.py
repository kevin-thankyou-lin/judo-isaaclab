...
def test_handover_camera_clockwise_roll_preserves_receiver_position():
    pose = _pose(0.4, -0.1, 0.9)
    rotated = _handover_target_with_camera_clockwise_roll(pose, np.deg2rad(32.0))
    np.testing.assert_allclose(rotated[:3], pose[:3], atol=0.0)
    assert np.linalg.norm(rotated[3:]) == pytest.approx(1.0)
    assert not np.allclose(rotated[3:], pose[3:])
    with pytest.raises(ValueError, match="in \\[0, 45\\] degrees"):
        _handover_target_with_camera_clockwise_roll(pose, -1.0e-6)
    with pytest.raises(ValueError, match="in \\[0, 45\\] degrees"):
        _handover_target_with_camera_clockwise_roll(pose, np.pi / 4.0 + 1.0e-6)
...
def test_handover_local_seat_translates_only_along_oriented_pad_depth_axis():
    pose = _handover_target_with_local_pitch(_pose(0.4, -0.1, 0.9), np.pi / 4.0)
    shifted = _handover_target_with_local_seat(pose, 0.018739788666255294)
    np.testing.assert_allclose(shifted[3:], pose[3:], atol=0.0)
    np.testing.assert_allclose(
        shifted[:3] - pose[:3],
        quaternion_rotate(pose[3:], [0.0, 0.0, 0.018739788666255294]),
        atol=1.0e-12,
    )
    with pytest.raises(ValueError, match="within 4 cm"):
        _handover_target_with_local_seat(pose, 0.040001)
...
def test_handover_local_pad_standoff_backs_out_along_affordance_axis():
    ...
    np.testing.assert_allclose(
        clear[:3] - pose[:3],
        -quaternion_rotate(pose[3:], [0.0, 0.0, 0.08]),
        atol=1.0e-12,
    )
