...
def test_pick_contact_advance_follows_demonstrated_open_approach_axis():
    pregrasp = _pose(0.4, 0.1, 0.9)
    grasp = _pose(0.406, 0.108, 0.9)

    advanced = _advance_pick_contact(pregrasp, grasp, 0.007)

    assert advanced[:3] == pytest.approx([0.4102, 0.1136, 0.9])
    assert advanced[3:] == pytest.approx(grasp[3:])
    assert _advance_pick_contact(pregrasp, grasp, 0.0) == pytest.approx(grasp)
    with pytest.raises(ValueError, match="pick approach advance"):
        _advance_pick_contact(pregrasp, grasp, 0.016)
